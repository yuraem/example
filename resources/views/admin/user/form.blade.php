...
<div class="form-group">
    ...
    <select name="role"  id="role" class="@error('paysystem') is-invalid @enderror form-control">
        @foreach($roles as $k => $role)
        <option value="{{$k}}" @if($k == old('$role', $user->role)) selected @endif >{{ $role }}</option>
        @endforeach
    </select>
    @error('role')
    <div class="alert alert-danger">{{ $message }}</div>
...
<div class="form-group">
    <label for="ref">Партнёр</label>
    <input name="ref"  id="ref" type="text" value="{{ old('ref', $user->ref) }}" class="@error('ref') is-invalid @enderror form-control">
