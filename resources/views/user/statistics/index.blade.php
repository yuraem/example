      ...
      <div class="card">
        <div class="card-header">
          <!-- <h3 class="card-title">Responsive Hover Table</h3> -->
            <!-- <div class="pull-left"> -->
                  <!-- <a href="{{ route('user.company.create') }}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Создать кампанию</a> -->
            <!-- </div> -->
            <!-- <div class="pull-right"> -->
            <div class="">  
                  <form action="{{ route('user.statistics.index', $user->id) }}" method="get">
                  <div class="row">
                    <div class="col-md-3">
              ...
              <tbody id="tbody-ajax">                              
                  @foreach($companys as $company)
                      <tr>
                          ...
                          <td>{{ $company->geo }}</td>                                  
                          <td>
                              <a href="{{ route('user.statistics.show', $company->id)}}" class="btn btn-info btn-xs"><i class="fa fa-eye">&nbsp; {{__('Статистика')}}</i> </a>                              
                          </td>
                      </tr>
                  @endforeach
              </tbody>
...
@section('page-js-script')
    ...
    <script type="text/javascript">
          $("#selectboxid").change(function() {
            var code = $(this).val();            
            $.ajax({ 
              url: "{{ route('user.statistics.showAjax', $user->id) }}?q="+code, 
              method: 'get',              
              dataType: 'json',
              async: false,              
              success: function(data){   
                ...
                let categories_options_html;
                  ...
                  $.each(data.data, (key, val) => {
                      ...
                      categories_options_html +=  `<td><a href="/user/statistics/`+ val.id +`" class="btn btn-info btn-xs"><i class="fa fa-eye">&nbsp; {{__('Статистика')}}</i> </a></td></tr>`;                     
                  }); 
