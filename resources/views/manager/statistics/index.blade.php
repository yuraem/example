      ...
      <div class="card">
        <div class="card-header">
          <!-- <h3 class="card-title">Responsive Hover Table</h3> -->
            ...
            <div class="">  
                  <form action="{{ route('manager.statistics.index', $user->id) }}" method="get">
                  <div class="row">
                    <div class="col-md-3">
              ...
              <tbody id="tbody-ajax">                              
                  @foreach($companys as $company)
                      <tr>
                          ...
                          <td>{{ $company->geo }}</td>                                  
                          <td>
                              <a href="{{ route('manager.statistics.show', $company->id)}}" class="btn btn-info btn-xs"><i class="fa fa-eye">&nbsp; {{__('Статистика')}}</i> </a>                              
                          </td>
                      </tr>
                  @endforeach
              </tbody>
...
@section('page-js-script')
    ...
    <script type="text/javascript">
          ...
          $("#selectboxid").change(function() {
            var code = $(this).val();            
            $.ajax({ 
              url: "{{ route('manager.statistics.showAjax', $user->id) }}?q="+code, 
              method: 'get',              
              dataType: 'json',
              async: false,              
              success: function(data){   
                ...
                let categories_options_html;
                  ...
                  $.each(data.data, (key, val) => {
                      ...
                      categories_options_html +=  `<td><a href="/manager/statistics/`+ val.id +`" class="btn btn-info btn-xs"><i class="fa fa-eye">&nbsp; {{__('Статистика')}}</i> </a></td></tr>`;                     
                  }); 
